...
import { ItemQuery, PostItemMutationData, DeleteItemMutationData, Variables, MenuItem } from ".";
...
const useItems = (categoryID: number | undefined) => {
    ...
    const [removeItem] = useMutation<DeleteItemMutationData, Variables>(DELETE_ITEM, {
        update(cache, { data }) {
            const { items: newItems } = cache.readQuery({
                query: GET_ITEMS,
